...
Cell *createCell(char content, int hasBomb)
{
...
void revealCell(Cell *grid[], int row, int col, int rows, int cols, int *gameOver, int *bombRevealed)
{
...
void placeBombs(Cell *grid[], int rows, int cols, int bombPositions[][2], int numBombs)
{
...
int allNonBombCellsRevealed(Cell *grid[], int rows, int cols)
{
    for (int i = 0; i < rows; i++)
    {
        Cell *current = grid[i];
...
void cleanup(Cell *grid[], int rows)
{
    for (int i = 0; i < rows; i++)
    {
        Cell *current = grid[i];
...
int game_logic()
{
    ...
    do
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            Cell *current = minesweeperGrid[i];
        ...
        do
        {
            ...
            if (row >= 0 && row < rows && col >= 0 && col < cols)
            {
                ...
                if (gameOver)
                {
                    ...
                    {
                        printf("\nCongratulations! Level %d completed.\n", level);
                    }

                    // Ask the user for options after game over
                    int choice;
                    re:
                    printf("\n\n1.Continue the current level\n");
                ...
                {
                    printf("\n\nCONGRATULATIONS! All non-bomb cells revealed.\n");
                    break;
                }
            }
            else
